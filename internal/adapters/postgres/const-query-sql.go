...
const (
	...
	checkDayLastTextTask   = `SELECT DATE_TRUNC('day', (SELECT team_text_answer.date FROM team_text_answer WHERE team_id = $1 ORDER BY team_text_answer.date DESC LIMIT 1)) = CURRENT_DATE`
)
